  ...
  <div class="subdevice-page">
    ...
    <div class="page-body">
      ...
      <div class="reading-card" :class="alarmLevel">
        <div class="gauge-area">
          <van-circle
            :rate="coPercent" :speed="60" :text="coLabel"
            :color="alarmLevel === 'alarm' ? '#EA4335' : alarmLevel === 'warn' ? '#FBBC04' : '#34A853'"
            :stroke-width="60" size="160"
          />
        </div>
        <div class="reading-detail">
          <span class="detail-val">{{ coValue }} ppm</span>
      ...
      <div class="section-card">
        ...
        <div class="setting-row">
          <div class="setting-label">CO告警</div>
        ...
        <div v-if="alarmEnabled" class="threshold-row">
          ...
          <van-stepper v-model="alarmThreshold" :min="10" :max="400" :step="10" />
    ...
    <van-popup v-model:show="showInfo" position="bottom" round :style="{ height: '40%' }">
      <div class="info-popup">
        <h3>设备信息</h3>
        <div class="info-row"><span>设备名称</span><span>一氧化碳探测器</span></div>
        <div class="info-row"><span>设备ID</span><span class="mono">CO-20240501</span></div>
        ...
        <div class="info-row"><span>电池电量</span><span>78%</span></div>
      </div>
    </van-popup>
  </div>
</template>
...
const coPercent = computed(() => Math.min(100, (coValue.value / 200) * 100))
const coLabel = computed(() => coPercent.value.toFixed(0) + '%')

const alarmLevel = computed(() => {
  if (!alarmEnabled.value) return 'normal'
  if (coValue.value >= alarmThreshold.value) return 'alarm'
  if (coValue.value >= alarmThreshold.value * 0.6) return 'warn'
  return 'normal'
})
...
.reading-card {
  background: $bg-color; border-radius: $radius-lg; padding: 24px 20px;
  box-shadow: $shadow-card; text-align: center;
  &.normal { background: $bg-color; }
  &.warn { background: #FFFDF5; }
  &.alarm { background: #FFF5F5; }
}
.gauge-area { display: flex; justify-content: center; margin-bottom: 12px; }
.reading-detail { display: flex; flex-direction: column; align-items: center; gap: 4px; .detail-val { font-size: 20px; font-weight: 700; color: $text-primary; } .detail-tag { font-size: 12px; padding: 2px 10px; border-radius: $radius-pill; &.normal { background: $success-light; color: $success-color; } &.warn { background: $warning-light; color: #B8860B; } &.alarm { background: $danger-light; color: $danger-color; } } }
.reading-time { text-align: center; font-size: 11px; color: $text-placeholder; margin-top: 12px; }
