  ...
  <div class="subdevice-page">
    ...
    <div class="page-body">
      ...
      <div class="status-card" :class="triggered ? 'triggered' : 'normal'">
        ...
        <div class="status-text">{{ triggered ? '⚠ 检测到破碎' : '正常' }}</div>
      ...
      <div class="section-card">
        ...
        <div class="setting-row">
          <div class="setting-label">玻璃破碎告警</div>
      ...
      <div class="section-card">
        ...
        <van-button block round type="primary" @click="onTest" :loading="testing">测试探测器</van-button>
        <div class="test-hint">使用专用测试器发出玻璃破碎频率声音，确认设备响应正常</div>
      </div>
      ...
      <div class="section-card">
        ...
        <div class="event-item" v-for="ev in events" :key="ev.id">
          <div class="event-icon" :class="ev.type">
            <van-icon :name="ev.type === 'break' ? 'warning-o' : 'checked'" size="12" color="#fff" />
          </div>
    ...
    <van-popup v-model:show="showInfo" position="bottom" round :style="{ height: '40%' }">
      <div class="info-popup">
        <h3>设备信息</h3>
        <div class="info-row"><span>设备名称</span><span>玻璃破碎探测器</span></div>
        <div class="info-row"><span>设备ID</span><span class="mono">GB-20240501</span></div>
        ...
        <div class="info-row"><span>电池电量</span><span>81%</span></div>
      </div>
    </van-popup>
  </div>
</template>
...
const events = ref([
  { id: 1, type: 'test', label: '功能测试', time: '今天 14:00' },
  { id: 2, type: 'test', label: '功能测试', time: '昨天 10:30' }
])
...
.event-icon { width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; &.break { background: rgba(234,67,53,0.2); } &.test { background: rgba(52,168,83,0.2); } }
