    ...
    <el-dialog v-model="addPkgVisible" title="添加套餐" width="900px" top="5vh">
      <div class="transfer-filter-bar">
        ...
        <el-select v-model="pkgTransferFilter.slot" placeholder="投放运营位" size="small" class="filter-select-md" clearable>
          <el-option label="活动位" value="activity" />
            ...
            <div v-for="cat in categories" :key="cat.key" :class="['transfer-category-item', { expanded: expandedCats.includes(cat.key) }]">
              ...
              <div class="transfer-cat-body">
                <span v-for="pkg in getCatAddedPkgs(cat.key)" :key="pkg.id" class="transfer-tag">
                  {{ pkg.name }}<span class="transfer-tag-close" @click="removeCatPkg(cat.key, pkg.id)">&times;</span>
                </span>
...
const allPkgList = [
  { id: 'PKG20220001', name: '事件录像7天', direction: '设备', category: '云存储套餐', price: '¥6.90/$0.95', type: 'cloud', region: 'domestic', app: 'qx', slot: 'activity' },
  { id: 'PKG20220015', name: '国内专享·极简云存', direction: '用户', category: '云存储套餐', price: '¥59.00', type: 'cloud', region: 'domestic', app: 'qx', slot: 'activity' },
  { id: 'PKG20220011', name: 'Overseas-Pro', direction: '设备', category: '云存储套餐', price: '$12.99', type: 'cloud', region: 'overseas', app: 'hm', slot: 'activity' },
  ...
  { id: 'PKG20220030', name: '30天全时云存储Pro版', direction: '设备', category: '云存储套餐', price: '¥29.90/$4.50', type: 'cloud', region: 'domestic', app: 'qx', slot: 'activity' }
]
