...
| 2026-06-26 | v1.1 更新视频生成规则：不限制图片张数、历史日期限1次、每日仅保留最新视频 | Kiro |
...
- 自动汇聚设备云端抓拍图片
- AI 智能分类（常规、宠物、老人、儿童）
- 支持一键生成短视频（每日限 2 次，不限制图片张数，历史未生成日期限 1 次，每日仅保留最新视频）
...
| 生成视频 | 用户选择图片后服务端合成短视频（默认模板：转场+配乐），每日限 2 次，不限制图片张数，历史未生成日期仅限 1 次，每日仅保留最新视频 |
...
```mermaid
flowchart TD
    A[点击「生成视频」] --> B{当日生成次数 < 2?}
    B -->|否| C[按钮置灰「今日已用完」]
    B -->|是| D{该日期有可用生成次数?}
    D -->|否| E[Toast「该日期已生成过视频」]
    D -->|是| F[进入图片选择模式]
    F --> G[勾选图片（不限张数）]
    G --> H[点击「开始生成」]
    H --> I[确认弹窗]
    ...
    I -->|确认| J[上传图片列表 → 服务端合成]
    J --> K{合成结果}
    K -->|成功| L[保存至视频列表（同日旧视频替换）]
    K -->|超时| M[Toast「视频生成超时」]
    K -->|失败| N[Toast「视频生成失败」]
    M --> O[不消耗当日次数]
    N --> O
    L --> P[消耗次数，Toast「视频生成成功」]
```
...
| R05 | 历史日期生成限制 | 历史未生成视频的日期仅支持生成 1 次（该日期已生成过则不可再生成） |
| R06 | 每日视频保留 | 每日仅保留最新一个视频，同日期新生成的视频替换旧视频 |
| R07 | 图片张数 | 不限制选择图片张数 |
| R08 | 视频模板 | 仅默认模板（基础转场+背景音乐），不可选择 |
| R09 | 视频合成 | 服务端合成，60s 超时，超时不消耗生成次数 |
| R10 | 视频有效期 | 生成后保留 30 天，过期自动清理，剩余 ≤ 3 天时红色警示 |
| R11 | 图片来源 | 从云端抓图存储获取，无云存储服务的设备显示空态 |
| R12 | 空态展示 | 全部无图片：「暂无设备抓拍图片」；单分类无图片：「暂无该类图片」 |
...
**图片选择模式**：点击「生成视频」进入。每张图片出现勾选框，选中后边框高亮。底部显示已选数量 + 缩略图预览条 + 「开始生成」按钮 + 「取消」按钮。不限制选择图片张数。

**生成视频**：校验当日生成次数和历史日期限制，确认后弹窗验证，提交图片列表至服务端合成。等待期间全屏遮罩 + spinner + 「视频生成中...」。成功后 Toast 提示并跳转视频列表，失败提示重试且不扣次数。若同日已有视频则自动替换旧视频。
...
| 边界态 — 历史日期不可生成 | 该日期已生成过视频 | 点击生成按钮 Toast「该日期已生成过视频」 |
