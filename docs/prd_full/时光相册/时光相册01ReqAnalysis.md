...
| 生成视频 | 用户选择一个分类下的多张图片，系统自动合成短视频（配乐+转场），每日限制 2 次 |
...
| F5 | 时光相册 | 生成视频 | P0 | 底部「生成视频」按钮，点击后选择图片 → 确认生成，每日 2 次限制 | **核心功能** |
...
| 场景编号 | 场景名称 | 前置条件 | 操作步骤 | 预期结果 | 状态变化 |
|------|------|------|------|------|------|
| N-201 | 首次生成视频 | 分类有 ≥3 张图片，今日生成 0 次 | 1. 点击底部「生成视频」2. 进入图片选择模式 3. 勾选 3-20 张图片 4. 点击「开始生成」5. 确认 | 上传图片列表 → 服务端合成 → 返回视频 → 自动保存至视频列表 | 今日剩余次数: 2→1 |
| N-202 | 第二次生成 | 今日已生成 1 次 | 同上流程 | 成功生成 | 今日剩余次数: 1→0 |
...
| E-202 | 图片不足 | 分类图片 < 3 张 | 点击「生成视频」 | Toast「至少选择 3 张图片才能生成视频」 |
...
| E-206 | 图片过大 | 所选图片总大小超限 | 提交前校验 | Toast「所选图片总大小超过限制，请减少图片数量」 |
